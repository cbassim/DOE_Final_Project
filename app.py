...
if submitted:
    try:
        ...
        # Reassign the list rather than mutating it in place. This makes the
        # session-state update explicit and avoids intermittent stale displays.
        st.session_state["history"] = [*st.session_state["history"], result]
...
# Form submission already causes a rerun, but this second rerun ensures the
# history table is rendered from the newly committed session state. Keeping
# st.rerun() outside the try block avoids treating its control-flow signal as
# an experiment error on older Streamlit versions.
if run_committed:
    st.rerun()
...
if history:
    ...
    st.caption(f"{len(student_df)} of {BUDGET} runs recorded.")
