...
if not st.session_state["session_started"] or st.session_state["model"] is None:
    st.warning("Enter your Student or Group ID in the sidebar, then click **Start / Refresh** to begin.")

st.subheader("Run an Experiment")
st.caption("Enter your own starting settings in real units. Nothing is preloaded as the recommended start. Decimal settings are allowed for local follow-up and re-boxing.")
...
if len(st.session_state["history"]) >= BUDGET:
    st.warning("You have reached the 45-run budget. You can still review and download your data.")
